...
const Utils = {
  ...
  getPlayers(message, channelId) {
    if (message) {
      ...
      match.splice(0,1)
      const players = []
      match.forEach(nameStr => {
        players.push(nameStr.match(/@<=([1-9_a-z_A-Z_=]+)=>/)[1])
      })
      const promiseList = [Utils.getAllUserMap()]
      if (message.includes('<-channel->')) {
        promiseList.push(Utils.getChannelUsersId(channelId))
      } else {
        promiseList.push(Promise.resolve(players))
      }
  ...
  // 通过 uid 获取名字
  getUserListName(userList) {
    return Utils.getAllUserMap().then( userMap => userList.map(uid => userMap[uid]))
  },
  // 获取全部用户，除去机器人
  getAllUserMap () {
    const userMap = {}
